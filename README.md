...
* https://github.com/apple/swift/pull/70910 fixes quadratic performance when scheduling jobs for tree cases.

Fast path of the isolated deinit has additional cost of about 15-20ns per object when copying task-local values, and 35-40ns when resetting task-local values. On the fast path copying task-local values is a no-op, while resetting requires extra work, which costs about 20ns per object regardless of the number of task-local objects.

Slow path of the isolated deinit has extra costs of about 115ns when enqueueing and about extra 140ns when dequeueing.
Resetting task-local values is a nop-op on the slow path, but copying incurs addition cost per value of 30-35ns when enqueueing and about 20ns when dequeueing.

When destroying a tree of objects with deinit isolated to the same actor, slow path is taken only once - for the root object.
For the rest of the objects last release happens on the expected actor and fast path is taken.

Assuming that it is common for trees of objects to be isolated to the same actor, and number of objects in the tree is larger than number of task-local values, copying task-local values is actually faster than resetting. Of course, an even faster option is to neither copy nor reset.

Async deinit does not have a fast path, and always creates a task, which costs about 270ns when enqueueing and about extra 315ns when dequeueing.
Copying task-local values is slightly faster for async deinit than for slow path of isolated deinit - about 25-27ns when enqueueing and about the same 20ns when dequeueing.
